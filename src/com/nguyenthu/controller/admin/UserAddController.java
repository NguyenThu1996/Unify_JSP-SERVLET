	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (eString != null) {
			if (eString.equals("1")) {
				...
				req.setAttribute("errMsgE", "This user name already in use");
			}
			if (eString.equals("2")) {
				req.setAttribute("user", user);
				req.setAttribute("errMsg","This username already in use!");
			}
		}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (userService.checkExistUsername(user.getUsername())) {
				HttpSession session = req.getSession();
                session.setAttribute("user", user);
				resp.sendRedirect(req.getContextPath() + "/admin/user/add?e=2");
			} else {
